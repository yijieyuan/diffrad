...
def getBinEdges(parameterValues, **kwargs):
    binCount = kwargs.get('binCount', 64)
    lowBound = kwargs.get('lowBound', 0)
    highBound = kwargs.get('highBound', 1)

    assert binCount is not None
    assert lowBound is not None
    assert highBound is not None

    binEdges = numpy.linspace(lowBound, highBound, num=binCount+1)
    binEdges[-1] += 1  # Ensures that the maximum value is included in the topmost bin when using numpy.digitize
    assert len(binEdges) > 1
    return binEdges
